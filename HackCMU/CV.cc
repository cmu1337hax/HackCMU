...
int main(){
    ...
    while(true){
        ...
        //open
        Mat const structure_elem = cv::getStructuringElement(MORPH_RECT, Size(3, 3)); //rect for morphologyEx
        Mat open;
        ...
        //Threshold
        Mat threshold;
        adaptiveThreshold(open, threshold, 255, CV_ADAPTIVE_THRESH_GAUSSIAN_C, CV_THRESH_BINARY_INV, 51, 20);

        cout<<"Thresholded!"<<nl;

        //close
        Mat const structure_elem1 = cv::getStructuringElement(MORPH_RECT, Size(25, 4)); //change size for word detection
        Mat close;
